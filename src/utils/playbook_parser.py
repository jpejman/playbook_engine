...
class PlaybookParser:
    ...
    def _try_json_parse(self, text: str, errors: List[str]) -> Optional[Dict[str, Any]]:
        ...
        json_match = self._find_json_in_text(text)
        if json_match:
            try:
    ...
    def _transform_canonical_to_legacy(self, canonical_data: Dict[str, Any]) -> Dict[str, Any]:
                ...
                for i, workflow in enumerate(workflows, 1):
                    if isinstance(workflow, dict):
                        ...
                        if "steps" in workflow and isinstance(workflow["steps"], list):
                            # Workflow has nested steps
                            for j, step in enumerate(workflow["steps"], 1):
                                if isinstance(step, dict):
                                    remediation_step = {
                                        "step_number": len(remediation_steps) + 1,
                                        "description": step.get("description", f"Step {j}"),
                                        "commands": step.get("commands", []),
                                        "verification": step.get("verification", ""),
                                        "evidence_based": step.get("evidence_based", False)
                                    }
                                    remediation_steps.append(remediation_step)
                        else:
                            # Workflow is a direct step
                            step = {
                                "step_number": i,
                                "description": workflow.get("description", workflow.get("workflow_name", f"Workflow step {i}")),
        ...
        if "post_remediation_validation" in canonical_data:
            prv = canonical_data["post_remediation_validation"]
            if isinstance(prv, dict):
                # Extract validation steps from dictionary
                if "validation_steps" in prv and isinstance(prv["validation_steps"], list):
                    # Convert validation steps to simple list of descriptions
                    verification_steps = []
                    for step in prv["validation_steps"]:
                        if isinstance(step, dict):
                            description = step.get("description", "Validation step")
                            verification_steps.append(description)
                        elif isinstance(step, str):
                            verification_steps.append(step)
                    legacy_playbook["verification_procedures"] = verification_steps
                else:
                    legacy_playbook["verification_procedures"] = ["Verify remediation was successful"]
            elif isinstance(prv, list):
                legacy_playbook["verification_procedures"] = prv
            else:
                legacy_playbook["verification_procedures"] = ["Verify remediation was successful"]
        else:
            legacy_playbook["verification_procedures"] = ["Verify remediation was successful"]
...
def parse_playbook_response(raw_text: str) -> Dict[str, Any]:
    """
    Backward compatibility wrapper for the new parser class.
    ...
    return parser.parse_playbook_response(raw_text)
...
def test_parser():
    ...
    # Test 1: Valid JSON response (legacy schema)
    print("\n1. Testing valid JSON response (legacy schema):")
    valid_json = '''{
        "playbook": {
            "title": "Test Playbook",
    ...
    result = parser.parse_playbook_response(valid_json)
    print(f"Parsed OK: {result['parsed_ok']}")
    print(f"Errors: {result['parse_errors']}")
    
    # Test 2: JSON with markdown fences (canonical schema)
    print("\n2. Testing JSON with markdown fences (canonical schema):")
    json_with_fences = '''```json
{
    ...
    "references": ["https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2025-10280"]
}
```'''
    
    result = parser.parse_playbook_response(json_with_fences)
    print(f"Parsed OK: {result['parsed_ok']}")
    print(f"Errors: {result['parse_errors']}")
    ...
    result = parser.parse_playbook_response(malformed_json)
    print(f"Parsed OK: {result['parsed_ok']}")
    print(f"Errors: {result['parse_errors']}")
...
Please review and let me know if you need changes.'''
    ...
    result = parser.parse_playbook_response(text_with_json)
    print(f"Parsed OK: {result['parsed_ok']}")
    print(f"Errors: {result['parse_errors']}")
    
    # Test 5: Empty response
    print("\n5. Testing empty response:")
    result = parser.parse_playbook_response("")
    print(f"Parsed OK: {result['parsed_ok']}")
    print(f"Errors: {result['parse_errors']}")
