"""
Pipeline executor for continuous_pipeline_v0_3_1
Version: v0.3.1
Purpose:
- Add structured output normalization and graded validation
- Support extraction, repair, and normalization layers
- Preserve strict canonical validation path
- Provide graded validation outcomes instead of binary fail
"""
...
class PipelineExecutor:
    def __init__(self):
        ...
        self.validation_grader = ValidationGrader()
    ...
    def execute(
        ...
        try:
            ...
            # Stage 5: multi-stage validation with extraction, repair, and normalization
            logger.info("Stage 5: Running multi-stage validation")
            validation_results = self._run_multi_stage_validation(response_text)
            
            extracted_response = validation_results.get("extracted_response")
            repaired_response = validation_results.get("repaired_response")
            normalized_response = validation_results.get("normalized_response")
            parse_passed = validation_results.get("parse_passed", False)
            repair_applied = validation_results.get("repair_applied", False)
            normalization_applied = validation_results.get("normalization_applied", False)
            canonical_validation_passed = validation_results.get("canonical_validation_passed", False)
            semantic_utility = validation_results.get("semantic_utility", {})
            validation_grade = validation_results.get("validation_grade", "HARD_FAIL")
            
            logger.info("Stage 5 complete: grade=%s parse_passed=%s repair_applied=%s normalization_applied=%s",
                       validation_grade, parse_passed, repair_applied, normalization_applied)
            ...
            # Determine pipeline status based on grade
            if validation_grade in ["STRICT_PASS", "NORMALIZED_PASS", "REPAIR_PASS"]:
                pipeline_status = "success"
                generation_status = "completed"
            elif validation_grade == "SEMANTIC_PARTIAL":
            ...
            result = {
                ...
                "model": resolved_model,
                "run_duration_seconds": run_duration_seconds,
                "validation_passed": canonical_validation_passed,
                "validation_grade": validation_grade,
                "parse_passed": parse_passed,
                "repair_applied": repair_applied,
                "normalization_applied": normalization_applied,
                "semantic_utility_flag": semantic_utility.get("has_useful_content", False),
                "extracted_response": extracted_response,
                "repaired_response": repaired_response,
                "normalized_response": json.dumps(normalized_response) if normalized_response else None,
                "evaluation_mode": evaluation_mode,
                "evaluation_batch_id": evaluation_batch_id,
                "evaluation_label": evaluation_label,
                "creator_script": resolved_creator_script,
            ...
            if normalized_response is not None:
                result["normalized_playbook"] = normalized_response
    ...
    def _run_multi_stage_validation(self, raw_response: str) -> Dict[str, Any]:
        """
        Run multi-stage validation with extraction, repair, and normalization.
        ...
        try:
            parsed_direct = json.loads(raw_response)
            results["parse_passed"] = True
            results["extracted_response"] = raw_response
            results["extraction_metadata"] = {"method": "direct_parse"}
        except json.JSONDecodeError:
            # Step 2: Extract JSON from raw text
            extraction_result = self.json_extractor.extract_with_context(raw_response)
            results["extraction_metadata"] = extraction_result.get("metadata", {})
            
            if extraction_result["success"]:
                extracted_json = extraction_result["extracted_json"]
                results["extracted_response"] = extracted_json
                
                # Try parse extracted JSON
                try:
                    parsed_direct = json.loads(extracted_json)
                    results["parse_passed"] = True
                except json.JSONDecodeError:
                    # Step 3: Repair JSON
                    repair_result = self.json_repair.repair_with_context(extracted_json)
                    results["repair_metadata"] = repair_result.get("metadata", {})
                    
                    if repair_result["success"]:
                        repaired_json = repair_result["repaired_json"]
                        results["repaired_response"] = repaired_json
                        results["repair_applied"] = True
                        
                        # Try parse repaired JSON
                        try:
                            parsed_direct = json.loads(repaired_json)
                            results["parse_passed"] = True
                        except json.JSONDecodeError:
                            # Repair failed to produce valid JSON
                            parsed_direct = None
                    else:
                        # Extraction succeeded but repair failed
                        parsed_direct = None
            else:
                # Extraction failed
                parsed_direct = None
        
        # If we have parsed JSON, proceed with normalization and validation
        if parsed_direct is not None:
            # Step 4: Normalize schema
            normalization_result = self.schema_normalizer.normalize_with_context(parsed_direct)
            results["normalization_metadata"] = normalization_result.get("metadata", {})
            
            if normalization_result["success"]:
                normalized_json = normalization_result["normalized_json"]
                results["normalized_response"] = normalized_json
                results["normalization_applied"] = normalization_result["metadata"].get("normalization_applied", False)
                
                # Step 5: Assess semantic utility
                semantic_utility = self.schema_normalizer.get_semantic_utility(normalized_json)
                results["semantic_utility"] = semantic_utility
                
                # Step 6: Run canonical validation
                try:
                    # Use the existing validation from generation_payload_builder
                    canonical_valid, _, validation_result = (
                        self.generation_payload_builder.validate_response(
                            json.dumps(normalized_json) if normalized_json else "{}"
                        )
                    )
                    results["canonical_validation_passed"] = canonical_valid
                    results["canonical_validation_result"] = validation_result
                except Exception as e:
                    logger.warning(f"Canonical validation failed: {e}")
                    results["canonical_validation_passed"] = False
                    results["canonical_validation_result"] = {"error": str(e)}
        
        # Step 7: Assign validation grade
        grade_result = self.validation_grader.grade_validation(
            raw_response=raw_response,
            extracted_response=results["extracted_response"],
            repaired_response=results["repaired_response"],
            normalized_response=results["normalized_response"],
            parse_passed=results["parse_passed"],
            repair_applied=results["repair_applied"],
            normalization_applied=results["normalization_applied"],
            canonical_validation_passed=results["canonical_validation_passed"],
            semantic_utility=results["semantic_utility"]
        )
        
        results["validation_grade"] = grade_result.get("grade", "HARD_FAIL")
        results["grade_metadata"] = grade_result
    ...
    def _persist_generation_run(
        ...
        # Determine status based on validation grade
        if validation_grade in ["STRICT_PASS", "NORMALIZED_PASS", "REPAIR_PASS"]:
            status = "completed"
            generation_source = "evaluation_pipeline_success"
        elif validation_grade == "SEMANTIC_PARTIAL":
            status = "partial"
            generation_source = "evaluation_pipeline_partial"
        else:
            status = "failed"
            generation_source = "evaluation_pipeline_failed"

        # Build error info if validation failed
        llm_error_info = None
        if not canonical_validation_passed:
        ...
        metadata = {
            ...
            "canonical_validation_passed": canonical_validation_passed,
            "semantic_utility": semantic_utility,
            "extraction_metadata": validation_results.get("extraction_metadata", {}),
            "repair_metadata": validation_results.get("repair_metadata", {}),
            "normalization_metadata": validation_results.get("normalization_metadata", {}),
            "grade_metadata": validation_results.get("grade_metadata", {}),
        }
