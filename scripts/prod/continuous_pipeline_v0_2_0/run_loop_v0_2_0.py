...
class RunLoopV020:
    ...
    def should_continue(self, last_summary: dict) -> bool:
        ...
        if last_summary['claimed'] == 0 and last_summary.get('enqueued', 0) == 0:
            print('Stopping: No items claimed and no new items enqueued in last cycle')
            return False
        return True
    ...
    def run(self) -> dict:
        ...
        while True:
            ...
            if self.wait_seconds > 0:
                print()
                print(f'Waiting {self.wait_seconds} seconds before next cycle...')
                time.sleep(self.wait_seconds)
        print()
        print('=== RUN LOOP FINAL SUMMARY ===')
...
def main():
    parser = argparse.ArgumentParser(description='Run Loop v0.2.0')
