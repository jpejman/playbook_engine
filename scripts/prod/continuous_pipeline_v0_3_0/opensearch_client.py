...
class OpenSearchClient:
    ...
    def search_candidates(self, from_offset: int = 0, page_size: int = 100, 
                         sort_strategy: str = 'cvss_recent') -> list[dict[str, Any]]:
        if sort_strategy == 'cvss_recent':
            sort = [
                {'metrics.cvssMetricV31.0.cvssData.baseScore': {'order': 'desc', 'unmapped_type': 'float'}},
                {'published': {'order': 'desc', 'unmapped_type': 'date'}},
            ]
        elif sort_strategy == 'recent_only':
            sort = [
                {'published': {'order': 'desc', 'unmapped_type': 'date'}},
            ]
        elif sort_strategy == 'oldest_first':
            sort = [
                {'published': {'order': 'asc', 'unmapped_type': 'date'}},
            ]
        elif sort_strategy == 'random_seed':
            import time
            ...
            sort = [
                {'_script': {
                    'type': 'number',
                    'script': {
                        'source': f'doc["_id"].value.hashCode() * {seed}',
                        'lang': 'painless'
                    },
                    'order': 'asc'
                }}
    ...
    def search_candidates_after(self, page_size: int = 100, 
                               search_after: list | None = None) -> tuple[list[dict[str, Any]], list | None, bool]:
        """
        Search for CVE candidates using search_after for pagination.
        
        Args:
            page_size: Number of results to return
            search_after: Cursor values from previous search for pagination
            
        Returns:
            Tuple of (candidates, next_cursor, end_of_corpus)
            - candidates: List of normalized candidate documents
            - next_cursor: Cursor values for next page, or None if end reached
            - end_of_corpus: True if no more results after this page
        """
        # Stable deterministic sort: published_date ASC, cve_id.keyword ASC
        sort = [
            {'published': {'order': 'asc', 'unmapped_type': 'date'}},
        ...
        candidates = [self._normalize_hit(hit) for hit in hits]
        
        # Determine if we've reached the end and get next cursor
        end_of_corpus = len(hits) < page_size
        next_cursor = None
        
        if hits and not end_of_corpus:
            # Get sort values from the last hit for next page
            last_hit = hits[-1]
            sort_values = last_hit.get('sort', [])
            if sort_values and len(sort_values) >= 2:
                next_cursor = sort_values
        
        return candidates, next_cursor, end_of_corpus
