...
def main():
    ...
    logger.info(f"Starting queue filler v0.2.1")
    logger.info(f"Persistent cursor mode: {ContinuousPipelineConfig.CP_FEED_USE_PERSISTENT_CURSOR}")
    if ContinuousPipelineConfig.CP_FEED_USE_PERSISTENT_CURSOR:
        logger.info(f"Feeder name: {ContinuousPipelineConfig.CP_FEED_CURSOR_FEEDER_NAME}")
        logger.info(f"Cursor page size: {ContinuousPipelineConfig.CP_FEED_CURSOR_PAGE_SIZE}")
