...
class QueueFeederService:
    ...
    def fill_from_opensearch(
        self,
        page_size: int,
        max_scan: int,
        target_enqueue: int,
        max_scan_windows: int = 10,
        max_total_scan: int = 5000,
        min_enqueue_required: int = 5,
        sort_diversification: bool = True
    ) -> QueueFillSummary:
        ...
        if self.use_persistent_cursor:
            logger.info(
                f"Using PERSISTENT CURSOR mode (feeder_name={self.feeder_name}, page_size={self.cursor_page_size})"
            )
            ...
            logger.info("Using LEGACY mode (offset-based pagination)")
    ...
    def _fill_with_legacy_pagination(
        self,
        page_size: int,
        max_scan: int,
        target_enqueue: int,
        ...
        while (
            scanned < max_total_scan
            and enqueued < target_enqueue
            and windows_scanned < max_scan_windows
        ):
            ...
            batch = self.os.search_candidates(
                from_offset=offset,
                page_size=page_size,
            ...
            logger.info(
                f"Window {windows_scanned}: scanned={window_scanned}, enqueued={window_enqueued}, "
            ...
            if window_enqueued == 0 and enqueued == 0 and windows_scanned > 1:
                ...
                logger.warning(
                    f"Starvation detected: scanned {scanned} CVEs across {windows_scanned} windows, "
        ...
        if enqueued == 0:
            ...
            logger.warning(
                f"Zero enqueue summary: windows_scanned={windows_scanned}, "
    ...
    def _fill_with_persistent_cursor(
        self,
        page_size: int,
        max_scan: int,
        target_enqueue: int,
        ...
        if cursor_state and cursor_state.get('completed_full_pass'):
            logger.warning(
                f"Full pass already completed for feeder {self.feeder_name}. "
        ...
        while scanned < max_total_scan and enqueued < target_enqueue:
            ...
            logger.info(
                f"Page processed: scanned={window_scanned}, enqueued={window_enqueued}, "
                f"skipped_queue={window_skipped_queue}, skipped_generation={window_skipped_generation}, "
